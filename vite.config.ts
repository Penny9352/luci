...
/**
 * 五个入口：`/` 中文、`/en/` 英文、`/contact.html` 联系我们，
 * `/redesign-preview/hero-v2.html` 首屏改版预览（浅色），
 * `/redesign-preview/hero-v2-dark.html` 同一版的深色实拍背景对比稿。
 *
 * （`redesign-preview/hero-v3.html` 还是本地未提交的草稿，故意没接进这份
 * 入口列表——之前手滑带进来一次，CI 上全新 checkout 找不到这个文件，
 * Rollup 直接报 "Could not resolve entry module" 构建失败。等这页定稿、
 * 连同用到的图一起提交了，再把这条加回来。）
 *
 ...
 *
 * 后三个都是独立的静态页（不接入 App.tsx / 组件体系）：contact.html 是
 * 页脚「联系我们」链过去的落地页，只有 LOGO + 一段简介 + 联系方式；
 * 两个 redesign-preview 页是为了能在 GitHub Pages 上留一个可分享的链接。
