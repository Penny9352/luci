...
export const simulation = {
   ...
   * 输入框的模式切换器。
   *
   * 产品 UI 里第二项叫「暴力模式」、描述写「综合最优结果返回」，两者都在
   * spec §9 的对外禁用清单里。对外用技术代称「多模型协同推理引擎」，
   * 「择优输出」是动作（≠ 被禁的「最优」，那是空头承诺）。
   */
  modes: [
    { name: '智能模式', desc: '自动选择最合适的单个模型处理你的请求', current: true },
    { name: '多模型协同推理引擎', desc: '同时调用多个模型作答，交叉验证后择优输出', current: false },
...
export const crossCheck = {
  stage: '01',
  stageName: '交叉验证',
  title: '一个问题，让多个 AI 一起思考',
  lead: '单个 AI 在编的时候，不会告诉你它在编。这不是能力问题——它只有一个视角，没法发现自己错了。',
  /** 技术代称写进句子里加重，不做标题上方的眉题 */
  engine: {
    pre: '自主研发的',
    name: '多模型协同推理引擎',
    post: '同时调用多个模型作答，交叉验证后择优输出。',
  },
